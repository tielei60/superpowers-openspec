...
- 用户要求编写或更新方案文档时，应实际创建或更新 `docs/solutions/<主题>.md`，不要只返回路径、模板或写作建议
- 用户只询问应该走哪个阶段或使用哪个命令时，只做路由判断，不擅自创建方案文档
...
- 方案文档写成业务系统设计文档，不写成 AI 技术说明书
- 每个功能点按“业务场景 -> 系统处理 -> 技术支撑”展开，先让业务人员理解系统做什么，再补接口、字段、任务、模型等技术细节
- 每个功能点说明：解决什么业务问题、系统怎么处理、异常情况怎么处理、业务价值是什么
- 优先使用“用户/业务人员……时，系统会……”的句式
- 技术词不是禁用词，不逐词列禁用清单；“事实、召回、向量、实体、关系、切片、上下文、模型”等词只是示例，不是穷举。出现技术词时，先解释业务含义和用户可见结果，再说明技术实现
- 使用人类易读语义，避免 AI 式套话；行为、规则、场景描述写成 WHO（具名角色）+ TRIGGER（触发）+ OUTCOME（可观测结果），不用"系统 / 数据 / 状态 / 事实"做主语（详见 `SKILL.md`）
- 正文（目标、设计、规则、验收标准）使用确定性语言；未确认事项写入"待确认问题"并暂停确认，不用"可能、也许、大概、或许、暂定"在正文中带过
