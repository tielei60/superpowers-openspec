...
- 存在未确认事项时，暂停并列出全部问题；等待用户明确确认后再生成或更新 OpenSpec change，不自行假设
...
方案文档和 OpenSpec 产物正文必须使用确定性语言：规则、行为、任务、验收标准要可执行、可验证。

- 不用"可能、也许、大概、或许、暂定"承载规范结论
- "如果/若"只用于明确条件分支；风险写成"若 X，则 Y"
- "应该"只能表示规范要求，不能表示猜测
- 未确认内容只放入"待确认问题"章节，并先暂停确认
